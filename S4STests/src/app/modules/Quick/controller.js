...
export async function doAddressDetails(from, to) {
    // add customClass props in Address component
...
export async function fillUpEnvelopeForm(pkgType, quantity, instruction, pickup_ready, pickup_closing, pickup_point, department, service_type) {
    // await page.waitFor(100);
    ...
    // await page.click(Selectors.buttons.ship);

    return true;
}

export async function fillUpBoxForm(pkgType, quantity, weight, length, width, height, instruction, pickup_ready, pickup_closing, pickup_point, department, service_type) {
    // await page.waitFor(100);
    await page.click(Selectors.selects.package_type);
    await page.select(Selectors.selects.package_type, pkgType);
    ...
    // await page.waitForSelector(".quick-shipment-ship-button", { timeout: 10000, visible: true })
    // await page.click(Selectors.buttons.ship);
...
export async function additionalServices(hold_for_pickup, non_conveyable, private_home_delivery, signature_required, trade_show_delivery, weekend_delivery, insurance, insurance_value) {
    ...
    if (insurance) {
        ...
        // await page.waitForSelector(".dicom-surcharge-input input[name=DCV]", { timeout: 10000, visible: true })
        await page.click(".dicom-surcharge-input input[name=DCV]");
        await page.type(".dicom-surcharge-input input[name=DCV]", insurance_value);
    } 

    // await page.click("label[for=DCV]");
    // await page.keyboard.press("Tab");
    // await page.waitForSelector(".dicom-surcharge-input");
    // await page.click("input[name=DCV]");
    // await page.type("input[name=DCV]", insurance_value);
